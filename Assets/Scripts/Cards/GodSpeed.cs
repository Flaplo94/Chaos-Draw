...
public class GodSpeed : MonoBehaviour, IAbilityBehavior
{
    ...
    [Header("Aura Visual (Animator created at runtime)")]
    ...
    [SerializeField] private bool auraUseSortingGroup = true;
    [SerializeField, Range(0f, 1f)] private float auraAlpha = 0.5f;
    ...
    private void CreateAuraHost()
    {
        ...
        auraHost.transform.localScale = Vector3.one;

        var playerSR = player.GetComponent<SpriteRenderer>();
        if (auraUseSortingGroup)
        {
            var sg = auraHost.AddComponent<SortingGroup>();
            if (playerSR)
            {
                sg.sortingLayerID = playerSR.sortingLayerID;
                sg.sortingOrder = playerSR.sortingOrder + 10;
            }
            else
            {
        ...
        auraSR = auraHost.AddComponent<SpriteRenderer>();
        auraSR.enabled = true;

        // Apply inspector alpha
